...
// enqueue when the selected service-instance queue total exceeds threshold.
//
...
//
// Per-(trace,span) state is freed at OnEnd. The DEE queue crosses traces, so
// EvictTrace does NOT touch it. It is keyed per service by default, or per
// simulated service instance when UseDEEQueueID is enabled.
type SBridgeHandler struct {
	...
	// UseDEEQueueID switches the cross-trace DEE queue key from ServiceID to
	// Event.DEEQueueID. The latter is generated by cmd/dee_instance_prep so
	...
	// service_id or simulated-instance queue_id -> pending DEE quadruples.
...
func (h *SBridgeHandler) deeQueueKey(ev *Event) uint32 {
	if h.UseDEEQueueID {
...
func (h *SBridgeHandler) enqueueDEE(queueID uint32, serviceID uint16, triple []byte, traceID uint64) {
	prev := 0
...
func (h *SBridgeHandler) EvictTrace(traceID uint64) {
	...
	// DEE queue is intentionally not touched (cross-trace instance state).
}
