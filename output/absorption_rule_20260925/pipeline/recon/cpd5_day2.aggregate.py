...
ROOT = Path("/home/ubuntu/cpd5/day2")
...
def main():
    ...
    for mode in MODES:
        for arm in ARMS:
            for shard in sorted(p.name for p in Path("/dev/shm/day2_shards").iterdir() if p.is_dir()):
    ...
    print(f"jobs aggregated : {njobs} / {len(MODES)*len(ARMS)*50}")
