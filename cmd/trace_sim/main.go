...
type config struct {
	...
	pressureSvcDepths   bool   // also emit the joint (service, depth) cells
...
func parseFlags() config {
	...
	flag.StringVar(&reversePolicy, "reverse-policy", "", "Enable reverse trusses: ttl, probability, inverse_depth, depth_linear, depth_quadratic, depth_cubic, depth_quartic, depth_ratio, or experimental upstream_pressure")
	...
	flag.BoolVar(&c.pressureSvcDepths, "pressure-service-depths", false, "With --checkpoint-pressure, also emit joint (service, depth) cells; enlarges the JSON")
	...
	c.reverse, checkpointErr = parseReverseConfig(c, reversePolicy, leafReject, reverseProbability, reverseTTLRange, reverseSeed, reverseExponent, reversePassCheckpoints)
	if c.reverse == nil && checkpointErr == nil {
		flag.Visit(func(f *flag.Flag) {
			if f.Name == "leaf-reject" || f.Name == "reverse-seed" || f.Name == "reverse-pass-checkpoints" {
	...
	if c.deeQueueIDs != "" && !isSBridgeFamily(c.mode) {
		fmt.Fprintln(os.Stderr, "error: --dee-queue-ids is only valid with --mode sbridge or sb3")
		os.Exit(2)
	}
	if c.deeDequeueOne && !isSBridgeFamily(c.mode) {
...
func makeHandler(c config, serviceName func(uint16) string, sourceFile func(uint64) string) (h bridge.Handler) {
	...
	case "sbridge":
		...
		h.DequeueOneDEE = c.deeDequeueOne
		h.DEEStats = c.deeStats
		if c.fpBits > 0 {
			h.FPBits = c.fpBits
		}
		if c.prefixLen > 0 {
	...
	case "sb3":
		...
		h := bridge.NewSB3Handler(c.checkpointDistance, c.prefixLen, c.bloomFP, logger)
		h.TopoOnly = c.topoOnly
		h.LehmerEE = c.lehmerEE
		h.UseDEEQueueID = c.deeQueueIDs != ""
		h.DequeueOneDEE = c.deeDequeueOne
...
func isSBridgeFamily(mode string) bool { return mode == "sbridge" || mode == "sb3" }
...
func main() {
	...
	if len(os.Args) > 1 && os.Args[1] == "histtable" {
		runHistogramTable(os.Args[2:])
		return
	}
	c := parseFlags()
	...
	if c.sizeHistograms != "" {
		...
		if isSBridgeFamily(c.mode) {
			c.deeStats = bridge.NewDEEQueueStats()
		}
	}
...
func runFromCorpus(c config, hist *sizeHistograms) []TraceMetrics {
	...
	if c.checkpointPressure != "" {
		...
		c.pressure = newCheckpointPressure(meta.Services, instancePath != "", c.pressureSvcDepths)
	}
	...
	if W > 1 && isSBridgeFamily(c.mode) {
		fmt.Fprintf(os.Stderr, "warning: --workers > 1 ignored for %s (cross-trace DEE state is order-dependent); running single-threaded\n", c.mode)
		W = 1
	}
